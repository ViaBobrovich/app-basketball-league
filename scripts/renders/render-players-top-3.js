...
function top3Points(section, playersList, gamesList) {
  ...
  for (const player of playersList) {
    ...
    for (const game of gamesList) {
      if (
        ...
        for (const log of game.logs) {
          if (log.playerId === player.id && log.action === "shot made") {
    ...
    if (games === 0 || points === 0) {
      ...
      playerIdPointsGamesList.push([player.id, (points / games).toFixed(2)]);
    }
  }
  ...
  for (let i = 0; i < 3; i++) {
    const myPlayer = playersList.find(
      (item) => item.id === playerIdPointsGamesList[i][0],
    );
...
function top3Rebounds(section, playersList, gamesList) {
      ...
      if (
        ...
        for (const log of game.logs) {
          if (
            ...
            (log.playerId === player.id && log.action === "rebound defensive")
          ) {
            rebounds += 1;
          }
        }
  ...
  for (let i = 0; i < 3; i++) {
    const myPlayer = playersList.find(
      (item) => item.id === playerIdReboundsGamesList[i][0],
    );
...
function top3shooting3PointsPercentage(section, playersList, gamesList) {
  ...
  for (let i = 0; i < 3; i++) {
    const myPlayer = playersList.find(
      (item) => item.id === playerIdshooting3PointsPercentageGamesList[i][0],
    );
